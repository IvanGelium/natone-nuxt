...
function handleClick() {
  ...
  const promiseOfData = getData('GET', 'https://jsonplaceholder.typicode.com/posts/1')
  promiseOfData
    .then(_data => addLog(createLog('micro', 'Данные пришли, обрабатываем промис')))
    .catch(_error => addLog(createLog('micro', 'Данные не удалось получить')))
    .finally(() => addLog(createLog('micro', 'Завершили обработку промиса')))
...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex flex-col gap-2">
      <div>
        Работа Event loop на примере запроса данных
      </div>
      <div>
        Цель: наглядно продемонстрировать порядок работы асинхронного кода JS
      </div>
      <div>
        Результат: лог операций, с обозначением из какой очереди пришла задача
      </div>
    </div>
